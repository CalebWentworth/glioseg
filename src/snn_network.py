...
class GroupNormNetwork(nn.Module):
    ...
    def __init__(self, in_channels ,out_chanels=2, beta=0.9):
        ...
        self.enc1 = SNNGroupConvBlock(in_channels, 16, beta, spike_grad)
        self.pool1 = nn.MaxPool3d(2)

        #encode l2
        self.enc2 = SNNGroupConvBlock(16, 32, beta,spike_grad)
        self.pool2 = nn.MaxPool3d(2)

        #encode l3
        self.enc3 = SNNGroupConvBlock(32, 64, beta, spike_grad)
        self.pool3 = nn.MaxPool3d(2)


        #bridge layer (bottom of the network)

        self.bridge = SNNGroupConvBlock(64,128, beta, spike_grad)

        #decode l1
        self.up1 = nn.ConvTranspose3d(128, 64, kernel_size=2, stride=2)
        self.dec1 = SNNGroupConvBlock(128, 64, beta, spike_grad)

        #decode l2
        self.up2 = nn.ConvTranspose3d(64, 32, kernel_size=2, stride=2)
        self.dec2 = SNNGroupConvBlock(64, 32, beta, spike_grad)

        #decode l3
    
        self.up3 = nn.ConvTranspose3d(32, 16, kernel_size=2, stride=2)
        self.dec3 = SNNGroupConvBlock(32,16, beta, spike_grad)
...
class SNNGroupConvBlock(nn.Module):

    def __init__(self,in_channels,out_chanels, beta, spike_grad, num_groups=2):
        
        super().__init__()
        self.block = nn.Sequential(
            nn.Conv3d(in_channels, out_chanels, kernel_size=3, padding=1),
            nn.GroupNorm(num_groups=num_groups, num_channels=out_chanels),
            snn.Leaky(beta=beta, spike_grad=spike_grad, init_hidden=True),
